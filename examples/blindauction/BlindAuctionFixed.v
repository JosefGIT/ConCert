...
Definition require_no_self_call ctx := required_false (ctx.(ctx_from) =? ctx.(ctx_contract_address))%address.
...
Definition find_bids_or_empty (key : Address) (map : FMap Address Bids) :=
    with_default [] (FMap.find key map).
...
Definition bid_call chain ctx state blinded_bid : option (State * list ActionBody) :=
    ...
    let updated_caller_bids := (new_bid::previous_caller_bids) in
...
Definition place_bid (state : State) (bidder : Address) (value : Amount) (fake : bool) (deposit : Amount) : bool * State :=
    ...
    if ((value <=? highest_bid') || fake || (deposit <? value))
    then (false, state)
    else
...
(* Aux. function for [reveal_call] *)
Fixpoint compute_refund caller (acc_refund : Amount) new_state caller_bids values fakes secrets : option (Amount * State) :=
    match caller_bids, values, fakes, secrets with
    | caller_bid::caller_bids', value::values', fake::fakes', secret::secrets' =>
        if (caller_bid.(blinded_bid) =? hash_bid value fake secret)%positive
        then
          let (success, updated_state) := place_bid new_state caller value fake caller_bid.(deposit) in
          let refund := caller_bid.(deposit) - (if success then value else 0) in
          compute_refund caller (acc_refund + refund) updated_state caller_bids' values' fakes' secrets'
        else
          None
    | [], [], [], []  => Some (acc_refund, new_state)
    | _, _, _, _ => None
    end.
...
Definition reveal_call chain ctx state values fakes secrets: option (State * list ActionBody) :=
    ...
    let caller := ctx.(ctx_from) in
    let caller_bids := find_bids_or_empty caller state.(bids) in
    let state' := state <| bids := (FMap.remove caller state.(bids)) |> in
    do to_refund_updated_state <- compute_refund caller 0 state' caller_bids values fakes secrets;
    let '(to_refund, updated_state) := to_refund_updated_state in
    Some (updated_state, [act_transfer caller to_refund]).

Definition withdraw_call chain ctx state : option (State * list ActionBody) := 
    do require_no_self_call ctx;
    do (require_no_money_sent ctx);
...
Definition receive (chain : Chain) (ctx : ContractCallContext)
  ...
  | Some withdraw => withdraw_call chain ctx state
...
Definition init (chain : Chain) (ctx : ContractCallContext) (setup : Setup)
  : option State :=
  do required_true (0 <? setup.(bidding_time))%nat;
  do required_true (0 <? setup.(reveal_time))%nat;
